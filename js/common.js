$(function () {
  ...
  function Accordion() {
    ...
    $(window).on("load resize", function () {
      var isMobile = window.matchMedia("screen and (max-width: 896px)").matches;

      if (isMobile) {
        $(".js_acd").removeClass("js_open");
        $(".js_acd_trg").stop(true, true).show(); // força visível, remove inline display:none
      } else {
        $(".js_acd_trg").stop(true, true).hide(); // desktop começa fechado
      }
    });
